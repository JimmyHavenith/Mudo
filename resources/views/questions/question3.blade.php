...
@section('title', 'quiz' )
	@section ('mainContent')
			...
			<div class="movie-banner-img">
				...
				<div class="quiz container">
					...
					<div class="yes col-md-6 col-sm-6 col-xs-6">
						<p><a title="Je répond positivement à cette question" href="/question4/{{ $tag }}">YES</a></p>
					</div>
		...
		<div class="movies-result container">
			<ul>
				@forelse($responseEt['results'] as $key => $value)
				...
				@foreach($responseOu['results'] as $key => $value)
					<li class="col-md-2 col-sm-3 col-xs-4 results-quiz-movies">
						<div class="proposal-image">
							<a title="Accéder au film {{ $value['title'] }}" href="#">
								<img src="https://image.tmdb.org/t/p/w780/{{ $value['poster_path'] }}" alt="affiche du film {{ $value['title'] }}" title="Accéder au film {{ $value['title'] }}"/>
							</a>
							<div class="overlay">
								<a title="{{ $value['title'] }}" href="/movies/{{ $value['id'] }}-{{ str_slug( $value['title'], '-') }}">
									<img src="../img/more.png" alt="Image pour en savoir davantage sur le film {{ $value['title'] }}" />
						...
						<div class="detector">
							<img class="bulle-detector" src="../img/bulle-blue.png" alt="Film correspondant à un seul critère" />
						</div>
						<div class="proposal-details">
							<a title="{{ $value['title'] }}" href="/movies/{{ $value['id'] }}-{{ str_slug( $value['title'], '-') }}">{{ $value['title'] }}</a>
						</div>
